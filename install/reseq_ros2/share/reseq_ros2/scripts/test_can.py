...
def print_results(t):
    ...
    def fq(x):
        return colored('OK', 'green') if x > (freq * 0.98) else colored('SLOW', 'red')
    ...
    for addr in messages.keys():
        ...
        for msg_id, received in messages[addr].items():
            print(
                f'+ {descriptions[msg_id]} ({hex(msg_id)}): {d[bool(received)]}',
                f'{received/t} Hz {fq(received/t)}',
                sep=' | ',
            )
            if received != 0:
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            'Verifies CAN connections to Rese.Q modules, that feedback messages are '
            'received and checks their frequency. If all messages are received, exits '
            'with the success code (independently from their frequency)'
        )
    )
