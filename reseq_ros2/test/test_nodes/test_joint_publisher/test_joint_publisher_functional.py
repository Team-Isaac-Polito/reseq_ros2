...
class TestJointPublisherFunctional(unittest.TestCase):
    ...
    def test_4_init_state(self):
        ...
        # Call init_state
        node.init_state(
            self.expected_params['modules'],
            self.expected_params['joints'],
        ...
        for mod in self.expected_params['modules']:
            ...
            if mod in self.expected_params['joints']:
                for x in node.states_from_type(rc.StateType.JOINT_FEEDBACK):
                    ...
                    self.assertEqual(node.position_states[(mod, x)].name, x + f'_{mod_id}_joint')
    ...
    def test_5_create_subs(self):
        ...
        for mod in self.expected_params['modules']:
            for topic in node.topics_from_type(rc.StateType.MOTOR_FEEDBACK):
                ...
                self.assertTrue(
                    any(sub.topic_name == sub_topic for sub in node.subscriptions),
                    f'Subscription not found for {sub_topic}',
                )
    ...
    def test_7_joint_feedback(self):
        ...
        self.assertAlmostEqual(
            node.position_states[(0x12, 'joint_y')].value, expected_angle, places=5
        )
    ...
    def test_8_end_effector_feedback(self):
        ...
        node.update_callback(
            int32_msg, 0x11, 'end_effector/pitch/feedback', rc.StateType.END_EFFECTOR_FEEDBACK
        )

        # Verify pitch position states
        expected_value_pitch = (
            (int32_msg.data - self.expected_params['arm_pitch_origin'])
            * rc.lsb_to_rads
            * self.expected_params['arm_pitch_gain']
            * (-1)
        )
        self.assertAlmostEqual(
            node.position_states[(0x11, 'arm_pitch')].value, expected_value_pitch, places=5
        )

        # Test case for end effector head pitch feedback
        int32_msg = Int32(data=650)
        node.update_callback(
            int32_msg, 0x11, 'end_effector/head_pitch/feedback', rc.StateType.END_EFFECTOR_FEEDBACK
        )

        # Verify head pitch position states
        expected_value_head_pitch = (
            int32_msg.data - self.expected_params['head_pitch_origin']
        ) * rc.lsb_to_rads
        self.assertAlmostEqual(
            node.position_states[(0x11, 'arm_head_pitch')].value,
            expected_value_head_pitch,
            places=5,
        )
        ...
        node.update_callback(
            int32_msg, 0x11, 'end_effector/head_roll/feedback', rc.StateType.END_EFFECTOR_FEEDBACK
        )

        # Verify head roll position states
        expected_value_head_roll = (
            int32_msg.data - self.expected_params['head_roll_origin']
        ) * rc.lsb_to_rads
        self.assertAlmostEqual(
            node.position_states[(0x11, 'arm_head_roll')].value, expected_value_head_roll, places=5
        )
    ...
    def test_9_broadcast_states(self):
        ...
        self.assertTrue(
            joint_states.header.stamp.sec > 0 or joint_states.header.stamp.nanosec > 0,
            'JointState timestamp is not set.',
        )
        ...
        for i in range(len(node.wheel_velocities)):
            node.create_subscription(
                TwistStamped, f'/diff_controller{i}/cmd_vel', twist_stamped_callback, 10
            )
        ...
        for i, vel in enumerate(node.wheel_velocities):
            ...
            self.assertTrue(
                twist_stamped_msgs[i].header.stamp.sec > 0
                or twist_stamped_msgs[i].header.stamp.nanosec > 0,
                'TwistStamped timestamp is not set.',
            )
