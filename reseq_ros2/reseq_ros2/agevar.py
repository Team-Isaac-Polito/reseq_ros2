...
class Agevar(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info('Agevar node successfully initialized')
    ...
    def remote_callback(self, msg: Twist):
        try:
            ...
            self.get_logger().error(f'Error in remote_callback: {str(e)}\n{traceback.format_exc()}')
    ...
    def yaw_feedback_callback(self, msg, module_num):
        try:
            ...
            self.get_logger().error(f'Error in yaw_feedback_callback: {str(e)}\n{traceback.format_exc()}')
    ...
    def kinematic(self, linear_vel, angular_vel, yaw_angle):
        try:
            ...
            self.get_logger().error(f'Error in kinematic calculation: {str(e)}\n{traceback.format_exc()}')
            raise
    ...
    def vel_motors(self, lin_vel, ang_vel, sign):
        try:
            ...
            self.get_logger().error(f'Error in vel_motors calculation: {str(e)}\n{traceback.format_exc()}')
            raise
