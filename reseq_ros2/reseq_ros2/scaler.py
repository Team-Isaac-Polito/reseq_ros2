...
class Scaler(Node):
    def __init__(self):
        ...
        try:
            ...
            self.get_logger().info("Scaler node started successfully")
    ...
    def agevarScaler(self, data: Twist):
        try:
            ...
            self.get_logger().error(f'Error in agevarScaler: {str(e)}\n{traceback.format_exc()}')
            raise
    ...
    def endEffectorScaler(self, data: EndEffector):
        try:
            ...
            self.get_logger().error(f'Error in endEffectorScaler: {str(e)}\n{traceback.format_exc()}')
            raise
    ...
    def scale(self, val, range):
        try:
            ...
            self.get_logger().error(f'Error in scale: {str(e)}\n{traceback.format_exc()}')
            raise
