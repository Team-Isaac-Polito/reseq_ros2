...
class AppGateway(Node):
    def __init__(self):
        ...
        self.get_logger().info('Service /ui/node_status is available for querying node status')
    ...
    def _ensure_node_running(self, module_id):
        ...
        if module_id not in self.node_launchers:
            return False, f'Module {module_id} is not configured'
        ...
        if module_id in self.hw_processes and self.hw_processes[module_id] is not None:
            ...
            if process.poll() is None:  # Process still alive
                self.get_logger().info(f'Node {node_name} is already running (PID: {process.pid})')
                return True, f'{node_name} is already active'
            else:
        ...
        try:
            ...
            self.get_logger().info(f'Node {node_name} is started (PID: {process.pid})')
            
            # Give node time to initialize and register its service (2 seconds for safety)
            time.sleep(2.0)
            
            return True, f'{node_name} is started successfully'
    ...
    def _stop_node(self, module_id):
        ...
        if module_id not in self.hw_processes or self.hw_processes[module_id] is None:
            self.node_states[module_id] = False
            return True, f'{node_name} is already stopped'
        ...
        try:
            ...
            try:
                ...
                self.get_logger().info(f'Node {node_name} is stopped with SIGTERM')
            except subprocess.TimeoutExpired:
                self.get_logger().warning(f'SIGTERM timeout for {node_name}, sending SIGKILL')
                os.killpg(os.getpgid(process.pid), signal.SIGKILL)
                process.wait(timeout=2.0)
                self.get_logger().info(f'Node {node_name} is stopped with SIGKILL')
            
            self.node_states[module_id] = False
            del self.hw_processes[module_id]
            return True, f'{node_name} is stopped successfully'
    ...
    def universal_callback(self, request, response, module_id):
        ...
        response.message = f'{module_id} is activated successfully'
        return response
    ...
    def handle_node_status(self, request, response):
        ...
        """
        self.get_logger().info('Query node status received')
