      ...
      int num_modules, int mk_version)
  {
    ...
    for (const auto &mapping : config)
    {
      ...
      for (int i = 1; i <= num_modules; ++i)
      {
        ...
        if (msg_mapping.length > 8)
        {
          throw std::runtime_error(
              "Invalid CAN mapping length for msg_id " + std::to_string(msg_mapping.id.msg_id) +
        ...
        for (const auto &field : mapping["fields"])
        {
          ...
          if (field_mapping.data_type == "float32")
          {
            if (field_mapping.offset + sizeof(float) > msg_mapping.length)
            {
              throw std::runtime_error(
                  "Invalid CAN mapping for msg_id " + std::to_string(msg_mapping.id.msg_id) +
                  ": field '" + field_mapping.name + "' exceeds payload length");
            }
          }
